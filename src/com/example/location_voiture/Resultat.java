...
public class Resultat extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
	 ...
	 private void find_and_modify_text_view() {
	        ...
	        button1.getBackground().setColorFilter(new LightingColorFilter(0xFFFFF, 0x003932));*/
...
//----------vitre-------
          if (check.isChecked()) {
              resultat = resultat + "," + check.getText();
...
//--------commentaire---------
     ...
     // int in=0;
      ...
      Vehicule vehiculeFromBdd = vBdd
       ...
       // Si le vehicule à bien été ajouté à la BDD, on affiche //les données du vehicule dans un Toast 
       
       if (vehiculeFromBdd == null) {
    	     if(r!="" &&re!=""&&res!=""&&resultat!=""&& chaine.compareTo("")!=0 && chaine1.compareTo("")!=0){
    	         
    	         // Création et insertion d'un vehicule
    	         Vehicule ve = new Vehicule(chaine,r,re,res,resultat,chaine1);
    	         
    	         vBdd.insertVehicule(ve);
    	         e.setText(null);
    	         ed.setText(null);
    	         Toast.makeText(Resultat.this, "L'etat de vehicule a été bien enregistré", Toast.LENGTH_SHORT).show();
    	        }
    	        else 
    	       	 Toast.makeText(Resultat.this, "Veuillez resseyer le remplissage", Toast.LENGTH_SHORT).show();
    	            r="";
    	            e.setText(null);
    	            ed.setText(null);
    	   ...
    	   Toast.makeText(Resultat.this, "Ce véhicule existe dèja dans la base de données", Toast.LENGTH_SHORT).show();
